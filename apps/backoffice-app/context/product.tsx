'use client'
import { createProduct, getProductDetail, getProducts } from "app/api/product.api";
...
export const ProductProvider = ({ children, productId }: any) => {
	...
	const [mappedProductData, setMappedProductData] = useState(initialAddProductValues)
	const [productPagination, setProductPagination] = useState(initialPaginationValue) 
	...
	const productFormMapper = (prodList?: any) => {
		...
		const productForm: IMappedProductForm = {
			generalInfo: {
				...
				categories: data?.categories && data.categories.map((item: any) => ({
					...item,
	...
	return (
		<ProductContext.Provider value={expose}>
			{children}
