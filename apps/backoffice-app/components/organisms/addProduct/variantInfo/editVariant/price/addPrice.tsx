...
import { priceConstraintSchema, priceInformationForm, priceTierSchema } from './schema';
...
const AddVariantPrice = (props: any) => {
	...
	const { register, handleSubmit, getValues, reset, formState: { errors }, unregister } = useForm({
	...
	return (
		<Box className="flex flex-col" component="form" onSubmit={handleSubmit(onSubmit)}>

			{/* Header Section */}
			<Box className="flex flex-col border-b pb-4">
				<Box className="flex flex-row justify-between items-center">
					<Typography className="text-2xl mb-2 mt-2 font-medium">{translate("product.addEmbeddedPrice")}</Typography>
				...
				>
					<Box className="flex flex-col px-4">
						<Box className="flex flex-col pl-8" gap={2}>
							<CustomFieldMapper formFields={priceInfoForm} formData={initialPriceData} register={register} errors={errors} />
						</Box>
						...
						<Box className="flex flex-col pl-8 mt-4" gap={2}>
							<CustomFieldMapper formFields={priceConstraintForm} formData={initialPriceData} register={register} errors={errors} />
						</Box>
					</Box>	
				</AccordianUnControlled>
				...
				>
					<Box className="px-4">
						...
						<Box className="flex flex-col pl-0 mt-4" gap={2}>
							{priceTiers.map((row: Array<any>, index) => (
								row.length > 0 &&
								<Box key={index} className="flex flex-row w-full items-center justify-between" gap={2}>
									<CustomFieldMapper 
										formFields={row} 
										formData={{}} 
										register={register} 
						...
						<Box className="pt-4">
							<CustomButton.Add title={translate("product.addPriceTier")} onClick={onAddPriceTier} />
						</Box>
