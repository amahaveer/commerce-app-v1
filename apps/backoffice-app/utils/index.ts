...
export const extractSetAtrributeOptions = (item: any, locale = "en-US") => {
	...
	return results.map((res) => ({
		value: res.key,
		label: res.labelAllLocales?.find((data: any) => data?.locale === locale)?.value,
	}))
}
...
export const getFilterOperatorOptions = (translate: ITranslateFunc): Array<IOptionPair> => [
	{ value: "and", label: translate("common.and") },
	{ value: "or", label: translate("common.or") },
]

export const getFilterAttributeFieldOptions = (translate: ITranslateFunc): Array<IOptionPair> => [
...
export const getPublishStatusOptions = () => [
	{ value: "published", label: "Publish", color: "#067446", bgColor: "#dbfae6" },
	{ value: "unpublished", label: "Unpublish", color: "#545978", bgColor: "#f0f1f5" },
]

export const convertToLabelValuePairs = (data: Array<string>) => data.map(item => ({
...
export const isValidImageUrl = (url: string) => /\.(jpg|jpeg|png|gif)$/.test(url);
...
export function formatDateTime(isoString: string) {
	...
	if (!month || !day || !year || !hours || !minutes || isNaN(hours) || isNaN(minutes)) {
	  return '--';
	}
...
export function formatCurrency(payload: IFormatCurrency={}) {
	const { currencyCode, centAmount, fractionDigits } = payload;
	if (!currencyCode || centAmount===undefined) return '--';
...
export const getBase64ImageDimensions = (base64Image: string): Promise<{ width: number; height: number }> => {
